 ...
 * Lupinus
 * 3.9.25
 * This is the script for the MusicManager.
...
public class MusicManager : MonoBehaviour
{
    ...
    //***Private Methods***//
    private void PlayMusic(Dictionary<string, MusicEntry> dictionary, string key, Transform spawnTransform)
    {
        if (dictionary.TryGetValue(key, out MusicEntry entry))
        {
        ...
        else Debug.LogWarning($"MusicManager didn't find a clip for key {key}.");
    }
    
    private void PlayRandomMusic(Dictionary<string, MusicEntry> dictionary, string[] keys, Transform spawnTransform) //enter like this: new string[] = {"key1","key2","key3"}
    {
    ...
    private Dictionary<string, MusicEntry> BuildDictionary(List<MusicEntry> list)
    {
        Dictionary<string, MusicEntry> dictionary = new Dictionary<string, MusicEntry>();
